...
export class Axiom {
  ...
  on<K extends keyof AxiomEventMap>(
    event: K,
    listener: (payload: AxiomEventMap[K]) => void
  ): () => void {
  ...
  async run(input: string, options: RunOptions = {}): Promise<RunResult> {
    if (!input.trim()) {
      throw new TypeError("AXIOM input cannot be empty.");
    }
      ...
      while (true) {
        ...
        if (rounds > maxToolRounds) {
          throw new ToolLoopError(
            `Tool execution exceeded the configured limit of ${maxToolRounds} rounds.`
          );
        }
  ...
  stream(
    ...
    import("./types.js").ModelStreamEvent
  > {
    if (!input.trim()) {
      throw new TypeError("AXIOM input cannot be empty.");
    }
    ...
    if (!this.model.stream) {
      throw new ModelError(
        `Model provider "${this.model.provider}" does not implement streaming.`
      );
    }
  ...
  private async *createStream(
    historyPromise: Promise<MemoryMessage[]>,
    input: string,
